...
class RecordFilter(GeoFilterSet):
    ...
    location_text = django_filters.Filter(field_name='location_text', method='filter_location_text')
    ...
    def filter_location_text(self, queryset, field_name, value):
    
        """ Method filter for location wise search

        e.g. /api/records/?record_type=44a51b83-470f-4e3d-b71b-e3770ec79772

        """
        return queryset.filter(location_text=value)
        # return queryset.filter(location_text__icontains=value)
